        ...
        <div className="w-full h-full flex flex-col relative overflow-hidden">
          ...
          <AnimatePresence>
                  ...
                  {[
                    ...
                    <button key={i} onClick={item.onClick} className="w-full bg-[#1C1C1E] p-3.5 rounded-2xl flex items-center gap-3.5 text-left border border-white/5 active:scale-95 transition-all">
                      ...
                      <div className="flex flex-col">
                        <p className="font-black text-[15px] leading-tight">{item.title}</p>
            ...
            {screen === "s2" && (
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25 }} className="absolute inset-0 bg-[#0F0F0F] z-50 flex flex-col">
                <header className="h-14 flex items-center justify-between px-4 border-b border-white/5">
                  <button onClick={() => setScreen("s1")} className="p-2 text-zinc-100"><ArrowLeft className="w-6 h-6" strokeWidth={3} /></button>
                ...
                <div className="flex-1 px-4 space-y-6 pt-6 overflow-y-auto">
                   <div className="flex flex-col items-center gap-4">
                     <div className="w-20 h-20 bg-[#252528] rounded-full flex items-center justify-center text-3xl font-black border border-white/5 relative">
                       P
                       ...
                       <div className="w-full space-y-3">
                         <div className="bg-[#1C1C1E] h-12 rounded-lg px-4 flex items-center justify-between border border-white/5">
                             <div className="flex items-center gap-3">
                               ...
                               <span className="font-black text-[15px]">Solana</span>
                             </div>
                           <ChevronDown className="w-4 h-4 text-zinc-500" />
                         </div>
                          <input value={data.name} onChange={e => setData({...data, name: e.target.value})} className="w-full bg-[#1C1C1E] h-12 rounded-lg px-4 border border-white/5 font-black text-[15px]" placeholder="Name" />
                        ...
                        {privateKey && (
                          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center py-2 px-1">
                            <span className="font-black text-[15px]">Account Address</span>
                 ...
                 <div className="p-4 pb-8 border-t border-white/5">
                   <button disabled={!privateKey} onClick={() => setScreen("s3")} className="w-full h-12 rounded-2xl bg-[#AB9FF2] text-black font-black text-[16px] disabled:opacity-50">Import</button>
                 </div>
              </motion.div>
            )}
            ...
            {screen === "editor" && (
              <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 25 }} className="absolute inset-0 bg-[#0F0F0F] z-50 flex flex-col">
                <header className="h-14 flex items-center justify-between px-4 border-b border-white/5">
                  <button onClick={() => setScreen("s1")} className="p-2 text-zinc-100"><ArrowLeft className="w-6 h-6" strokeWidth={3} /></button>
                  <div className="font-black text-[17px]">Edit Mock Data</div>
                  <div className="w-10" />
                </header>
                ...
                <div className="p-4 pb-8 border-t border-white/5"><button onClick={handleSave} className="w-full h-12 rounded-2xl bg-[#AB9FF2] text-black font-black">Save Changes</button></div>
              </motion.div>
            )}
          </AnimatePresence>
                ...
                <div className="p-4 flex flex-col">
                  <header className="h-12 flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2 cursor-pointer" onClick={() => setScreen("s1")}>
                      <div className="w-8 h-8 bg-[#252528] rounded-full flex items-center justify-center font-black text-[12px]">{data.badgeCount}</div>
                      <span className="font-black text-[17px]">{data.homeName}</span>
                      <Copy className="w-4 h-4 text-zinc-500" strokeWidth={3} />
                    </div>
                    <div className="flex items-center gap-4 text-zinc-500">
                      <Search className="w-6 h-6 stroke-[3]" />
                      <div className="w-6 h-6 border-[2.5px] border-current rounded-[4px] relative flex items-center justify-center overflow-hidden">
                        <div className="absolute top-0 left-0 w-2 h-2 border-t-[2.5px] border-l-[2.5px] border-zinc-950" />
                        <div className="absolute top-0 right-0 w-2 h-2 border-t-[2.5px] border-r-[2.5px] border-zinc-950" />
                        <div className="absolute bottom-0 left-0 w-2 h-2 border-b-[2.5px] border-l-[2.5px] border-zinc-950" />
                        <div className="absolute bottom-0 right-0 w-2 h-2 border-b-[2.5px] border-r-[2.5px] border-zinc-950" />
                        <Maximize2 className="w-4 h-4 stroke-[3]" />
                      </div>
                    </div>
                  </header>
                    <div className="flex flex-col items-center mb-8">
                      <div className="text-[48px] font-black mb-1" style={{ letterSpacing: '-1.5px' }}>${data.bal}</div>
                  ...
                  <div className="grid grid-cols-4 gap-3 mb-8">
                    {[{i:<QrCode strokeWidth={3}/>,l:'Receive'}, {i:<Send strokeWidth={3}/>,l:'Send'}, {i:<Repeat strokeWidth={3}/>,l:'Swap'}, {i:<DollarSign strokeWidth={3}/>,l:'Buy'}].map((a, i) => (
                      <div key={i} className="bg-[#1C1C1E] aspect-square rounded-2xl flex flex-col items-center justify-center gap-2 cursor-pointer active:scale-90 transition-transform">
                        <div className="text-[#AB9FF2]">{a.i}</div><span className="text-[12px] font-black text-zinc-400">{a.l}</span>
                      </div>
                    ))}
                  </div>
                  {showBanner && (
                    <div className="bg-[#1C1C1E] rounded-[18px] p-4 mb-4 flex items-center gap-4 relative border border-white/5">
                      ...
                      <p className="text-[14px] font-black leading-snug pr-4">{data.banner}</p>
                      <X onClick={() => setShowBanner(false)} className="absolute top-2 right-2 text-zinc-500 w-4 h-4 cursor-pointer" strokeWidth={3} />
                    </div>
                  )}
                    <div className="bg-[#1C1C1E] rounded-[20px] p-4 flex items-center justify-between border border-white/5 active:bg-[#252528] transition-colors cursor-pointer" onClick={copyAddr}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col"><span className="font-black text-[17px]">{data.tokName}</span><span className="text-[13.5px] text-zinc-500 font-black">{data.tokAmt}</span></div>
                      </div>
                      <div className="flex flex-col items-end"><span className="font-black text-[17px]">${data.tokUsd}</span><span className={`text-[13.5px] font-black ${data.tokChg.startsWith('-') ? "text-[#FF5C33]" : "text-[#00FFA3]"}`}>{data.tokChg.startsWith('-') ? "-$" : "+$"}{data.tokChg.replace('-', '')}</span></div>
                    </div>
                    
                    <div className="flex items-center justify-center gap-2 mt-6 opacity-50 hover:opacity-100 transition-opacity cursor-pointer">
                       <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" className="text-zinc-500"><line x1="4" y1="8" x2="20" y2="8" /><line x1="4" y1="16" x2="20" y2="16" /><circle cx="15" cy="8" r="2" fill="#0F0F0F" stroke="currentColor" strokeWidth="3" /><circle cx="9" cy="16" r="2" fill="#0F0F0F" stroke="currentColor" strokeWidth="3" /></svg>
                       <span className="text-[15px] font-black text-zinc-500">{data.manage}</span>
                    </div>
                </div>
              )}
              {activeTab !== "home" && <div className="flex-1 flex items-center justify-center text-zinc-500 font-black text-sm uppercase tracking-widest">Mock Screen</div>}
            </div>
            <nav className="h-[72px] bg-[#0F0F0F] border-t border-white/5 flex items-center justify-around shrink-0 pb-2">
              {[
                { id: "home", icon: <Home className={activeTab === "home" ? "fill-current" : ""} strokeWidth={3} /> },
                { id: "grid", icon: <LayoutGrid strokeWidth={3} /> },
                { id: "swap", icon: <ArrowRightLeft strokeWidth={3} /> },
                { id: "activity", icon: <Clock strokeWidth={3} /> },
                { id: "search", icon: <Search strokeWidth={3} /> }
              ].map(t => (
